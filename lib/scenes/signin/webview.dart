...
class SigninWebview extends WebviewScaffold {
  static const queries = {
    ...
    'scope': 'read write',
    'redirect_uri':
        'https://phwk9v3rhh.execute-api.ap-northeast-1.amazonaws.com/default/esaAuth',
    'response_type': 'code'
  };
  ...
  void addSigninCompleteWatcher(BuildContext context) {
    ...
    webviewplugin.onStateChanged.listen((WebViewStateChanged state) {
      if (state.type == WebViewState.finishLoad) {
        final token = Uri.parse(state.url)
            .queryParameters['esa_access_token']; // クラッシュしそうな雰囲気を感じる
        // ドメインやパスも検証したほうが良いかも
        if (token != null) {
          // null安全他にいい方法がありそう
